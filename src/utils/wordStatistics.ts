 ...
 */
export const calculateLearningStats = async (): Promise<LearningStats> => {
 ...
 * Get unit statistics
 */
export const getUnitStats = async (unitId: string) => {
  const data = await getAllData();
...
export const getWordReviewStats = (word: Word) => {
  ...
  return {
    ...
    needsReview:
      !word.mastered ||
      (word.lastReviewTime && now - word.lastReviewTime > oneWeek),
  };
};
