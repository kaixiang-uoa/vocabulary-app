import { ImportWordData, ImportUnitData, ImportCompleteData, ImportData } from '../types';
...
export const parseCompleteJSON = (jsonContent: string): ImportCompleteData | null => {
  try {
    const data = JSON.parse(jsonContent);
...
export const parseImportContent = (content: string, fileType: string): ImportData => {
  switch (fileType.toLowerCase()) {
    case 'csv':
      ...
      if (firstLine) {
        const parts = firstLine.includes('\t') ? firstLine.split('\t') : firstLine.split(',');
        if (parts.length >= 3) {
