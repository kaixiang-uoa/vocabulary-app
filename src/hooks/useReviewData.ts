...
import { getAllData } from '../utils/wordUtils';
...
export const useReviewData = ({ 
  ...
  const loadData = useCallback(async (targetUnitId: string) => {
    ...
    if (lastLoadedUnitId.current === targetUnitId && (now - lastLoadTime.current) < 100) {
      console.log('useReviewData: Skipping duplicate request for unitId:', targetUnitId);
      return;
    }
    ...
    if (loadingPromise.current && lastLoadedUnitId.current === targetUnitId) {
      console.log('useReviewData: Waiting for existing load promise for unitId:', targetUnitId);
      return loadingPromise.current;
    }

    console.log('useReviewData: Loading data for unitId:', targetUnitId, 'timestamp:', now);
