...
export const useAutoPlay = (options: UseAutoPlayOptions = {}): UseAutoPlayReturn => {
  const {
  ...
  const [pronunciationDelay, setPronunciationDelayState] = useState(() => loadSettings().delay);
  ...
  const setPronunciationDelay = useCallback((delay: number) => {
