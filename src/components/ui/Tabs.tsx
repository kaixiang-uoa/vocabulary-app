...
const Tabs: React.FC<TabsProps> = ({
  ...
  const [currentActiveKey, setCurrentActiveKey] = React.useState(activeKey ?? defaultActiveKey);
  ...
  const tabPanes = children ? React.Children.toArray(children).filter(
    (child): child is React.ReactElement<TabPaneProps> => 
      React.isValidElement(child) && child.type === TabPane
  ) : [];

  const activePane = tabPanes.find(pane => pane.key === currentActiveKey) || tabPanes[0];
  const activeItem = tabItems.find(item => item.key === currentActiveKey) || tabItems[0];
        ...
        {(items ? tabItems : tabPanes).map((item) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              className={`px-6 py-3 text-base font-medium border-b-2 transition-all duration-200 ${
                isActive
